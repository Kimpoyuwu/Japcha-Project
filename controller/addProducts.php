...
if ($_SERVER["REQUEST_METHOD"] == "POST") {
    ...
    if(isset($_POST['editProd'])){
        ...
        // Insert into the database
        $sql = "UPDATE product SET image_url = ?, product_name = ?, description = ?, category_id = ? WHERE product_id = ?";
        $stmt = mysqli_prepare($con, $sql);

        if ($stmt) {
             // If no file is uploaded, the default value for image_url should be applied
        mysqli_stmt_bind_param($stmt, "sssii", $new_img_name, $productname, $description, $category, $prodID);
        mysqli_stmt_execute($stmt);
